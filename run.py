...
#GSD
gsd_writer = hoomd.write.GSD(filename='trajectory_prod.gsd',trigger=hoomd.trigger.Periodic(100000),mode='ab')
...
old_positions = np.genfromtxt("old_positions.txt")
...
swim_pressure = SwimPressure(sim,active,gamma=1.0,gamma_r=3.0,position_old=old_positions)
...
gsd_writer_log = hoomd.write.GSD(filename='log_prod.gsd',trigger=hoomd.trigger.Periodic(1000),mode='ab',filter=hoomd.filter.Null(), log=logger)
